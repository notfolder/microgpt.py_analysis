...
# 3,000パターン生成
python main.py --total-target 3000

# 12,000パターン生成
python main.py --total-target 12000

# 30,000パターン生成（最大）
python main.py --total-target 30000
```

**指定可能範囲:** 600 ～ 30,000パターン
...
各カテゴリ10パターンのみ生成し、動作確認と生成時間の推定を行います。

```bash
# 特定の目標数でテスト
python main.py --test --total-target 3000
```
...
総目標数を指定すると、各カテゴリの**生成速度に応じて自動的に目標数を配分**します。

**配分ロジック:**
- **速いカテゴリ**（文字列操作、スライス記法等）→ 多くのパターンを配分
- **遅いカテゴリ**（組み込み関数、標準ライブラリ等）→ 少ないパターンを配分
- **最小保証:** 各カテゴリ最低60パターンを保証

### デフォルト配分 (6,000パターン)

| カテゴリ | 目標数 | 速度 | 内容 |
|---------|---------|------|------|
| 数値計算 | ~570 | 普通 | 算術演算、べき乗、複素数 |
| ビット演算 | ~950 | 速い | シフト、AND/OR/XOR |
| 文字列操作 | ~1,200 | 速い | メソッド、結合、分割 |
| データ構造 | ~1,160 | 速い | リスト、辞書、セット |
| 真偽値・条件式 | ~1,160 | 速い | 比較、論理演算、三項演算子 |
| 組み込み関数 | ~60 | 非常に遅い | len, sum, max, min等 |
| 型変換 | ~60 | 非常に遅い | int, str, list, tuple等 |
| スライス記法 | ~1,470 | 最高速 | スライス、繰り返し、連結 |
| 標準ライブラリ | ~60 | 非常に遅い | import、math、sys |
| その他・応用 | ~80 | 遅い | 複合パターン |
| **合計** | **~6,000** | - | - |
...
3. **動的温度ブースト**: 重複率50%超過時に温度を自動上昇（最大1.2）
4. **ネガティブプロンプト**: 最近20パターンを「避けるべき例」として提示
5. **テンプレートローテーション**: 100バッチごとに表現を変更
...
2. **類似パターン検出**: Levenshtein距離（編集距離）< 5 で類似と判定（緩和して多様性を確保）
3. **最適化**: 文字数グループ化 + ハッシュプリフィルタで比較回数を削減

**重複率モニタリング:**
- カテゴリ内進捗表示で10パターンごとに重複率を表示
- カテゴリ完了時に最終重複率を表示
- 重複率50%超で温度パラメータを自動上昇

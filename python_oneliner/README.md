...
| カテゴリ | 目標数 | 内容 |
|---------|---------|---------|
| 数値計算 | 7,000 | 算術演算、べき乗、ビット演算、複素数 |
| 文字列操作 | 7,000 | スライス、メソッド、結合、分割 |
| データ構造 | 7,000 | リスト、辞書、セット、タプル |
| 組み込み関数 | 7,000 | len, sum, max, min, sorted, map, filter等 |
| 標準ライブラリ | 1,500 | import文（os, sys, math, re, gc） |
| その他・応用 | 500 | 複数カテゴリの組み合わせ |
| **合計** | **30,000** | |

**配分の考え方:**
- 主要4カテゴリはサンプルプールが豊富でバリエーションを生成しやすい
- 標準ライブラリは16文字制約でパターンが限定されるため1,500に抑制
- その他・応用は複合的で難度が高いため500に限定

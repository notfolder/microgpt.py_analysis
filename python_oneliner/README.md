...
- 目標: 30,000パターン
- カテゴリ: 7カテゴリ（数値計算、文字列操作、データ構造、内包表記、組み込み関数、標準ライブラリ、その他・応用）
...
合計推定時間 |          |    30000 |            |     77:17:00

推定結果:
  - 30,000パターン生成予想時間: 77:17:00
  - 約 77.3 時間
```
...
| カテゴリ | 目標数 | 内容 |
|---------|--------|------|
| 数値計算 | 5,000 | 算術演算、べき乗、ビット演算、複素数 |
| 文字列操作 | 5,000 | スライス、メソッド、結合、分割 |
| データ構造 | 5,000 | リスト、辞書、セット、タプル |
| 内包表記 | 5,000 | リスト/辞書/セット内包表記、ジェネレータ式 |
| 組み込み関数 | 5,000 | len, sum, max, min, sorted, map, filter等 |
| 標準ライブラリ | 3,000 | import文（os, sys, math, re, gc） |

...
| カテゴリ | 目標数 | 内容 |
|---------|---------|---------|
| 数値計算 | 5,000 | 算術演算、べき乗、複素数、16進数・8進数 |
| ビット演算 | 2,000 | シフト演算、論理演算(AND/OR/XOR)、ビット反転 |
| 文字列操作 | 5,000 | メソッド、結合、分割、大文字小文字変換 |
| データ構造 | 5,000 | リスト、辞書、セット、タプル、ネスト |
| 真偽値・条件式 | 3,000 | 比較演算、論理演算、三項演算子、in/is演算子 |
| 組み込み関数 | 4,000 | len, sum, max, min, sorted, map, filter等 |
| 型変換 | 2,000 | int, str, list, tuple, set, float, bool等 |
| スライス記法 | 2,000 | リスト/文字列スライス、繰り返し、連結 |
| 標準ライブラリ | 1,000 | import文（os, sys, math, re, gc） |
| その他・応用 | 1,000 | 複合パターン、関数ネスト |
| **合計** | **30,000** | |

**配分の考え方:**
- 10カテゴリに分散することで各カテゴリの拡大率を低減（50-100倍程度）
- 焦点が明確になり、LLMが特定のパターンに集中しやすい
- ビット演算、型変換、真偽値・条件式、スライス記法を独立カテゴリ化
- 標準ライブラリは16文字制約でパターンが限定されるため1,000に抑制
- 重複リスクが10カテゴリに分散され、生成の持続性が向上
